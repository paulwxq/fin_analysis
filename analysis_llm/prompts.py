...
约束要求：
1. 必须且只能输出 JSON 块。
2. 每条新闻必须是摘要形式，长度严格控制在 {max_chars} 字符以内。
3. 正面新闻列表最多包含 {limit_pos} 条最重要记录，负面新闻列表最多包含 {limit_neg} 条最重要记录。
4. 如果搜索结果不足，按实际数量返回；如果完全没有相关新闻，请在摘要中说明“未找到相关新闻”，不要留空。
"""
...
3. 'capital_flow' 必须是详细的描述性字符串。如果当前没有获取到资金流向数据，请填写 "暂无数据" 或 "数据不足"，严禁输出 0 或任何非字符串类型。
"""

KLINE_AGENT_SYSTEM = """你是一名技术分析专家。
我将提供一张中国股票 {stock_code} 的 **月K线图**。请基于图片进行分析。
...
CHECKER_SYSTEM = """你是一名冷静、严谨的数据规格质检员。
