# "平底锅"形态选股策略设计文档 (v2.2 - High Performance)
...
| `stock_flatbottom_preselect` | **写入** | 筛选结果存储（TRUNCATE + INSERT） | 见 2.3 节表结构 |
...
-- 创建索引
CREATE INDEX IF NOT EXISTS idx_flatbottom_sql_score ON stock_flatbottom_preselect(score DESC);
CREATE INDEX IF NOT EXISTS idx_flatbottom_sql_updated_at ON stock_flatbottom_preselect(updated_at);

-- 添加表注释
COMMENT ON TABLE stock_flatbottom_preselect IS '平底锅形态候选股票SQL筛选结果（每个股票只保留最新一条记录）';
...
COMMENT ON COLUMN stock_flatbottom_preselect.current_price IS '当前价格（元）';
COMMENT ON COLUMN stock_flatbottom_preselect.history_high IS '历史高点价格（默认120个月内，元）';
COMMENT ON COLUMN stock_flatbottom_preselect.glory_ratio IS '辉煌度：历史高点 / 历史低点，反映股票曾经的涨幅倍数';
COMMENT ON COLUMN stock_flatbottom_preselect.drawdown_pct IS '回撤幅度（%）：(当前价 - 历史高点) / 历史高点 × 100，负数表示跌幅';
COMMENT ON COLUMN stock_flatbottom_preselect.box_range_pct IS '箱体振幅（%）：近期波动幅度，(近期最高 - 近期最低) / 近期最低 × 100';
...
2. **数据更新策略**
   - 使用 `INSERT ... ON CONFLICT UPDATE` 实现 TRUNCATE + INSERT
   - `created_at`: 首次入选时间（不更新）
   - `updated_at`: 每次筛选时自动更新
...
2. **日常运行**: 直接运行筛选（自动 TRUNCATE + INSERT）
   ```bash
...
```
阶段1: SQL 粗筛 (数据库层)
  ├─ 输入: stock_monthly_kline 表（全市场 3000+ 只股票）
  ├─ 处理: 窗口函数计算 8 个核心指标
  ├─ 过滤: 多条件筛选 + 综合评分
...
利用 PostgreSQL 窗口函数计算 8 个核心指标：

| 指标 | 计算方法 | 阈值 | 金融含义 |
|:---|:---|:---|:---|
| **历史高点** | `MAX(high) OVER (120个月)` | - | 曾经辉煌的证据 |
| **辉煌度** | `历史高点 / 历史低点` | ≥ 3.0 | 涨幅倍数 |
| **回撤幅度** | `(当前价 - 历史高点) / 历史高点` | < -40% | 深度回调 |
| **箱体振幅** | `(近期最高 - 近期最低) / 近期最低` | < 50% | 底部横盘 |
| **波动率比** | `近期标准差 / 历史标准差` | < 0.60 | 情绪冷却 |
| **价格位置** | `(当前价 - 近期最低) / 箱体高度` | 0.05-0.80 | 避免追高/破位 |
| **数据完整性** | `COUNT(*) OVER (...)` | ≥ 60 | 确保历史可追溯 |
| **最低价格** | `close` | ≥ 3.0元 | 排除仙股 |
...
StockMetrics AS (
    SELECT
        ...
        MAX(high) OVER (
            PARTITION BY code
            ORDER BY month
            ROWS BETWEEN {history_lookback} PRECEDING AND CURRENT ROW
        ) AS history_high,
        ...
        MAX(high) OVER (
            PARTITION BY code
            ORDER BY month
            ROWS BETWEEN {recent_lookback} PRECEDING AND CURRENT ROW
        ) AS recent_high,
        ...
        STDDEV(close) OVER (
            PARTITION BY code
            ORDER BY month
            ROWS BETWEEN {history_lookback} PRECEDING AND {recent_lookback_plus_1} PRECEDING
...
ScoredCandidates AS (
    SELECT
        ...
        (
            -- 回撤越深，得分越高（归一化到0-40分）
            GREATEST(0, (ABS(drawdown_pct) - {min_drawdown_abs}) / {min_drawdown_abs} * 40) +
...
class FlatbottomScreener:
    ...
    def _build_sql_query(self) -> str:
        ...
        return sql_template.format(
            ...
            recent_lookback_plus_1=cfg['RECENT_LOOKBACK'] + 1,
...
**决策**: 在 v2.2 中移除了全局时间过滤条件 `WHERE month >= NOW() - INTERVAL '{history_lookback} months'`
...
def _save_to_database(self, results: pd.DataFrame, preset: str) -> int:
    ...
    try:
        # TRUNCATE + INSERT SQL（INSERT ... ON CONFLICT UPDATE）
        upsert_sql = """
            INSERT INTO stock_flatbottom_preselect (
                code, name, current_price, history_high, glory_ratio,
        ...
        logger.info(f"✓ 成功写入/更新 {inserted_count} 条记录到数据库")
...
PRESETS = {
    'conservative': {
        ...
        'MIN_GLORY_RATIO': 4.0,       # 最小辉煌度（历史高点/历史低点）
...
def validate_config(config: dict) -> None:
    ...
    assert config['MIN_GLORY_RATIO'] >= 1.0, "MIN_GLORY_RATIO 必须 >= 1.0"
...
def print_config(config: dict) -> None:
    ...
    print(f"  最小辉煌度:          {config['MIN_GLORY_RATIO']:.1f}")
    print(f"  最低股价:            {config['MIN_PRICE']:.1f} 元")
...
| `MIN_GLORY_RATIO` | float | 3.0 | 4.0 | 2.5 | 最小辉煌度（历史高点/历史低点） |
| `MIN_PRICE` | float | 3.0 | 5.0 | 2.0 | 最低股价（元，排除仙股） |
...
2026-01-27 09:30:17 | INFO     | selection:425 | ✓ 成功写入/更新 82 条记录到数据库
```
...
**默认行为**:
- ✅ 结果自动写入数据库（TRUNCATE + INSERT，自动更新）
...
parser.add_argument('--min-price', type=float, help='最低股价（元）')
...
-- 查询 Top 10 股票
SELECT code, name, current_price, glory_ratio, drawdown_pct, score
FROM stock_flatbottom_preselect
ORDER BY score DESC
LIMIT 10;
...
```csv
code,name,current_price,history_high,glory_ratio,drawdown_pct,box_range_pct,volatility_ratio,price_position,score,slope,r_squared
300444.SZ,双杰电气,8.25,26.80,4.52,-69.2,38.5,0.45,0.42,85.3,0.0082,0.65
002823.SZ,凯中精密,10.50,29.50,3.85,-64.4,42.1,0.52,0.38,78.6,0.0045,0.58
600482.SH,中国动力,18.30,48.20,3.92,-62.0,45.8,0.48,0.51,75.2,-0.0023,0.61
```
...
3. **TRUNCATE + INSERT**: 数据库自动更新，无需清空操作
